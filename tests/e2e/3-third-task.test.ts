...
describe('Task 3', () => {
  ...
  it('Should create a ticket in third party app 1', async () => {
    ...
    const { status, body: ticket } = await request(app).post(`/create-ticket/ThirdPartyApp1/${problem.id}`).send({
      problemId: problem.id,
      myStatus: AppOneTicketStatus.RUNNING,
      count: 1,
      owner: 'csTeam',
  ...
  it('Should create a ticket in third party app 2', async () => {
    ...
    await request(app).post(`/change-status/${ProblemStatus.READY}`).send();
    const { status, body: ticket } = await request(app).post(`/create-ticket/ThirdPartyApp2/${problem.id}`).send({
